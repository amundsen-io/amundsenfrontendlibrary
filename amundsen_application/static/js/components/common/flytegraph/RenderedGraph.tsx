 ...
 * component.
 */
export class RenderedGraph<T> extends React.Component<
    RenderedGraphProps<T>,
    RenderedGraphState
> {
    ...
    constructor(props: RenderedGraphProps<T>) {
        super(props);
        this.containerClickHandler = new DragFilteringClickHandler(
                        ...
                        {nodes.map(node => (
                            ...
                            >
                                <Node
                                    ...
                                    onEnter={this.onNodeEnter.bind(
                                        this,
                                        node.id
                                    )}
                                    onClick={this.onNodeClick.bind(
                                        this,
                                        node.id
                                    )}
                                    onLeave={this.onNodeLeave.bind(
                                        this,
                                        node.id
                                    )}
    ...
    private onNodeEnter = (id: string) => {
        const hoveredNodes: { [id: string]: boolean } = {
    ...
    private onNodeSelect = (id: string) => {
        ...
        // new selection
        if (this.props.selectedNodes && this.props.selectedNodes.indexOf(id) > -1) {
            return this.clearSelection();
        }

        this.props.onNodeSelectionChanged &&
            this.props.onNodeSelectionChanged([id]);
    };
