...
export function getLoggedInUser() {
  return axiosInstance.get(`/api/auth_user`)
...
export function getUser(userId: string, index?: number, source?: string) {
  ...
  return axiosInstance.get(`/api/metadata/v0/user?${queryParams}`)
...
export function getUserOwn(userId: string) {
  return axiosInstance.get(`/api/metadata/v0/user/own?user_id=${userId}`)
...
export function getUserRead(userId: string) {
  return axiosInstance.get(`/api/metadata/v0/user/read?user_id=${userId}`)
